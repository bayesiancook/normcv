...
int main (int argc, char* argv[])   {
    ...
    for (int rep=0; rep<nrep; rep++)    {
        ...
        double truel = model.GetSiteLogCV(n,m);
        double truel0 = model.GetSiteLogCV0(n,m);
        if (relative)   {
            truel -= truel0;
        }
        if (persite)    {
        ...
        double var = 0;
        double ess = 0;
        double isl = model.GetSiteISLogCV(n,m,nsample,var,ess);
        if (relative)   {
            isl -= truel0;
        }
        if (persite)    {
    ...
    cout << "sis" << '\t' << p << '\t' << nsample << '\t' << meaness << '\t' << meantruel << '\t' << meanisl << '\t' << bias << '\t' << meanestbias << '\t' << meanerr << '\t' << meanesterr << '\t' << meanderr << '\t' << meandesterr << '\t' << time << '\n';
}
